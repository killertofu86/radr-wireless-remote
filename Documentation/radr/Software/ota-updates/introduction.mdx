...
│                                                                      │
│  ┌─────────────────────┐         ┌─────────────────────────────────┐│
│  │   Firmware Update   │         │      Filesystem Update          ││
│  │                     │         │                                 ││
│  │  firmware.bin       │         │  littlefs.bin                   ││
│  │  └─ Application     │         │  ├─ /registry.json              ││
│  │     code            │         │  └─ /protocols/*.json           ││
│  │                     │         │     └─ Buttplug.io device specs ││
│  └─────────────────────┘         └─────────────────────────────────┘│
│                                                                      │
│  Update Server: Supabase Storage                                     │
│  URL Pattern: {SERVER}/master/{firmware|littlefs}.bin                │
└─────────────────────────────────────────────────────────────────────┘
```
...
│ menu         │
└──────┬───────┘
       │ User selects "Update Device"
       ▼
┌──────────────┐     WiFi not      ┌──────────────┐
│   update     │◄──────────────────│ update.wifi  │
│   (check)    │    connected      │   (portal)   │
└──────┬───────┘                   └──────────────┘
       │
       ├─── hasFilesystemUpdate ───►┌──────────────┐
       │                            │   update.    │
...
```
┌──────────────────┐     ┌──────────────────┐     ┌──────────────────┐
│  OTA downloads   │────▶│  LittleFS        │────▶│  initRegistry()  │
│  littlefs.bin    │     │  partition       │     │  loads on boot   │
│                  │     │  updated         │     │                  │
└──────────────────┘     └──────────────────┘     └──────────────────┘
                                                           │
                                                           ▼
